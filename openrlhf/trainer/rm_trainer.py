...
def drop_mask(c_mask, chosen_response_lengths):
    # 兼容 list / (B,1) / (B,) 等多种形状与 dtype
    crl = torch.as_tensor(chosen_response_lengths, device=torch.cuda.current_device()).view(-1).long()  # (B,)
    
    
    B, L = c_mask.shape

    # 为了安全，统一把 mask 当作 0/1 的整型来做长度计算
    c_mask_int = (c_mask != 0).to(torch.int64)

    for i in range(B):
        valid_len = int(c_mask_int[i].sum().item())          # prompt+response 的总有效长度

        resp_len = int(crl[i].item())
        prompt_len = max(valid_len - resp_len, 0)            # 只数 prompt 的 token 个数

        # 找到非 pad 段的起点（c_mask == 1 的第一处下标）
        # 注：不管是左 pad 还是右 pad，这里都成立，因为非 pad 段是连续的一段 1
        first_nonpad_idx = int((c_mask_int[i] != 0).to(torch.float32).argmax().item())

        # prompt 的起止（连续一段）：[p_start, p_start + prompt_len)
        p_start = first_nonpad_idx
        # 将 prompt 段尽量均分成 5 段；若 prompt_len < 5，就分成 prompt_len 段（每段 1 个）
        num_chunks = min(5, prompt_len)
        base = prompt_len // num_chunks

        # 随机选一段
        seg_idx = int(torch.randint(low=0, high=num_chunks, size=(1,), device=torch.cuda.current_device()).item()) # [0,1,2,3,4]
        # 该段相对 prompt 起点的偏移：i*base + min(i, rem)
        offset = seg_idx * base

        seg_start = p_start + offset
        seg_end = seg_start + (base if seg_idx < (num_chunks-1) else prompt_len)

        c_mask[i, seg_start:seg_end] = 0
        # 注意：这里直接在 c_mask 上原地修改即可，后续 loss 计算会使用更新后的 mask
...
class RewardModelTrainer(ABC):
    ...
    def fit(self, args, consumed_samples=0, num_update_steps_per_epoch=None):
        ...
        for epoch in range(start_epoch, self.epochs):
            ...
            for data in self.train_dataloader:
                if str(self.method).startswith('nce'):
                    chosen_ids, c_mask, reject_ids, r_mask, margin, fake_prompt_response_input_list, chosen_response_lengths, reject_response_lengths = data
                else:
                ...
                # ========= 主前向（真 P + 真 R）=========
                chosen_reward, reject_reward, aux_loss, output_PR = self.concatenated_forward(
                    self.model, chosen_ids, c_mask, reject_ids, r_mask
                )  # 期望 shape: [B]

                # debug_preview_batch(self.tokenizer, chosen_ids, c_mask, reject_ids, r_mask, file='output/data_log/my_own_template_train.txt', show_special=True)

                if self.compute_fp32_loss:
                    chosen_reward = chosen_reward.float()
                    reject_reward = reject_reward.float()

                B = chosen_reward.shape[0]
                ...
                if str(self.method).startswith('nce'):
                    ...
                    for ind, fake_prompt_response_input in enumerate(fake_prompt_response_input_list[:self.n_fake_prompt]):
                        chosen_ids_, c_mask_, reject_ids_, r_mask_ = fake_prompt_response_input

                        chosen_ids_ = chosen_ids_.squeeze(1).to(torch.cuda.current_device())
                        c_mask_ = c_mask_.squeeze(1).to(torch.cuda.current_device())
                        reject_ids_ = reject_ids_.squeeze(1).to(torch.cuda.current_device())
                        r_mask_ = r_mask_.squeeze(1).to(torch.cuda.current_device())

                        # 中等难度，最后一条样本使用-1/5样本
                        if (self.nce_hard_level == 'middle') and (ind == 2):
                            c_mask_ =  drop_mask(c_mask_, chosen_response_lengths)
                            r_mask_ =  drop_mask(r_mask_, reject_response_lengths)

                        elif self.nce_hard_level == 'hard':
                            c_mask_ =  drop_mask(c_mask_, chosen_response_lengths)
                            r_mask_ =  drop_mask(r_mask_, reject_response_lengths)

                        # with torch.no_grad():
                        f_chosen_reward_, f_reject_reward_, f_aux_loss, f_output_PR = self.concatenated_forward(
                            self.model, chosen_ids_, c_mask_, reject_ids_, r_mask_
                        )
                        if self.nce_grad_type == 'two_head_grad':
                            # 使用nce head输出logit
                            f_chosen_reward = f_output_PR['nce_chosen_logit']
                            f_reject_reward = f_output_PR['nce_rejected_logit']
                        elif self.nce_grad_type == 'rm_wo_grad':
                            # 使用rm head输出logit，rm head不参与梯度传播
                            f_chosen_reward = f_output_PR['nce_chosen_logit_wo_grad']
                            f_reject_reward = f_output_PR['nce_rejected_logit_wo_grad']
                        elif self.nce_grad_type == 'rm_w_grad':
                            # 使用rm head输出logit，rm head参与梯度传播
                            f_chosen_reward = f_chosen_reward_
                            f_reject_reward = f_reject_reward_

                        if self.compute_fp32_loss:
                            f_chosen_reward = f_chosen_reward.float()
                            f_reject_reward = f_reject_reward.float()

                        fake_chosen_rewards.append(f_chosen_reward)
                        fake_reject_rewards.append(f_reject_reward)

                    if self.nce_grad_type == 'two_head_grad':
                        gt_logits_chosen = output_PR['nce_chosen_logit']
                        gt_logits_reject = output_PR['nce_rejected_logit']
                    elif self.nce_grad_type == 'rm_wo_grad':
                        gt_logits_chosen = output_PR['nce_chosen_logit_wo_grad']
                        gt_logits_reject = output_PR['nce_rejected_logit_wo_grad']
                    elif self.nce_grad_type == 'rm_w_grad':
                        gt_logits_chosen = chosen_reward
                        gt_logits_reject = reject_reward

                    # [B, K]，K = 1(真配对) + len(fake)
                    logits_chosen = torch.stack([gt_logits_chosen] + fake_chosen_rewards, dim=1)
                    logits_reject = torch.stack([gt_logits_reject] + fake_reject_rewards, dim=1)

                    # 1. 减去均值，强调相对差异 2. 温度缩放
                    logits_chosen = (logits_chosen - logits_chosen.mean(dim=1, keepdim=True)) / self.tau
                    logits_reject = (logits_reject - logits_reject.mean(dim=1, keepdim=True)) / self.tau
                    ...
                    # 行内交叉熵：提升 I(P;Y|R)
                    cince_chosen = F.cross_entropy(logits_chosen, targets)
                    cince_reject = F.cross_entropy(logits_reject, targets)
                    cince_loss = 0.5 * (cince_chosen + cince_reject)
                    loss_nce = cince_loss
                    lambda_ = self.lambda_cmi
                    # mixtral
                    ...
                    loss = preference_loss + lambda_ * loss_nce + aux_loss * self.args.aux_loss_coef
                ...
                #                     n_preview=2, show_special=True, file='output/data_log/text_nce.txt')


                # *********************基于希尔伯特投影近似CMI估计算法*********************

                # a_logit = output_R_Only['a_logit']
                # # c_mask_R和r_mask_R分别代表chosen与rejected的mask，mask的只剩response
                # c_mask_R_only = output_R_Only['c_mask_R_only']
                # r_mask_R_only = output_R_Only['r_mask_R_only']
                # a_logit_claped = torch.clamp(a_logit, -self.clap_cap, self.clap_cap)


                # chosen_reward, reject_reward, aux_loss, output_PR = self.concatenated_forward(
                #     self.model, chosen_ids, c_mask, reject_ids, r_mask, 
                #     estimation_var_sample_num=self.estimation_var_sample_num,
                #     estimation_var_sample_rate=self.estimation_var_sample_rate,
                #     c_mask_R_only=c_mask_R_only, r_mask_R_only=r_mask_R_only,
                # )
                ...
                #     ce_a = -F.logsigmoid(a_logit - torch.Tensor(margin).to(a_logit.device)).mean()
                #     sigmoid_a = F.sigmoid(a_logit - torch.Tensor(margin).to(a_logit.device)).mean()
                # else:    
                #     ce_a = -F.logsigmoid(a_logit_claped).mean()                    
                #     sigmoid_a = F.sigmoid(a_logit_claped).mean()

                # second_order_correction = 0
                # if "estimation_var" in output_PR:
                #     estimation_var = output_PR['estimation_var']
                #     sigmoid_dd = F.sigmoid(a_logit_claped)*(1-F.sigmoid(a_logit_claped))*(1-2*F.sigmoid(a_logit_claped))
                #     second_order_correction = 0.5 * sigmoid_dd * estimation_var
                #     # grad2_modification = self.grad2_modification(estimation_var)
                #     cmi_correct_loss = -torch.log( F.sigmoid(a_logit_claped) + second_order_correction).mean()
                # else:
                #     cmi_correct_loss = ce_a
                ...
                # # 基于希尔伯特投影近似的loss
                # # loss = (1+self.lambda_cmi)*preference_loss - self.lambda_cmi * cmi_correct_loss + aux_loss * self.args.aux_loss_coef
                # # 基于InfoNCE近似的loss
                # # loss = preference_loss + loss_nce + aux_loss * self.args.aux_loss_coef
                # loss = preference_loss + aux_loss * self.args.aux_loss_coef
                # *********************基于希尔伯特投影近似CMI估计算法*********************
                ...
                if str(self.method).startswith('nce'):
                    logs_dict = {
                        ...
                        # "ce_a": ce_a.item(),
                        # "sigmoid_a": sigmoid_a.item(),
                        "bt_loss": preference_loss.item(),
                        "acc": acc,
                        # "ce_a": ce_a.item(),
                        # "sigmoid_a": sigmoid_a.item(),
                        "cince_chosen": cince_chosen.item(),
                        "cince_reject": cince_reject.item(),
                        "cince_loss": cince_loss.item(),
                    }
                else:
                    logs_dict = {
                        "lr": self.scheduler.get_last_lr()[0],
                ...
                # if self.estimation_var_sample_num>0:
                #     logs_dict['second_order_correction'] = second_order_correction.mean().item()
                if self.aux_loss:
    ...
    def evaluate(self, eval_dataloader, steps=0):
        ...
        with torch.no_grad():
            ...
            for data in eval_dataloader:
                ...
                # debug_preview_batch(self.tokenizer, chosen_ids, c_mask, reject_ids, r_mask, file='output/data_log/eval_stage.txt', show_special=True)
    ...
    def concatenated_forward(self, model, chosen_ids, c_mask, reject_ids, r_mask, **kwargs):
        ...
        output_PR = {
            "nce_chosen_logit": output['log_y_r_values_reward'][: chosen_ids.shape[0]],
            "nce_rejected_logit": output['log_y_r_values_reward'][chosen_ids.shape[0]:],
            "nce_chosen_logit_wo_grad": output['nce_logits_wo_grad'][: chosen_ids.shape[0]],
            "nce_rejected_logit_wo_grad": output['nce_logits_wo_grad'][chosen_ids.shape[0]:]
        }
        ...
        #     b_square_sum = 0
        #     for b_score in output['b_score_reward']:  # 采样数
        #         B = len(b_score)
        #         b_delta = b_score[:chosen_ids.shape[0]]-b_score[chosen_ids.shape[0]:]
        #         b_delta_mean_zero = b_delta - b_delta.mean()
        #         b_square_sum += torch.square(b_delta_mean_zero)

        #     estimation_var = b_square_sum / self.estimation_var_sample_num
        #     output_PR['estimation_var'] = estimation_var
        # # 解包输出
        ...
        return chosen_rewards, rejected_rewards, aux_loss, output_PR
