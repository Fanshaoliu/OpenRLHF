...
def preprocess_data(
    ...
    if apply_chat_template:
        if prompt_key and (prompt_key in data.keys()):
...
class RewardDatasetRaw(Dataset):
    ...
    def __getitem__(self, idx):
        data, prompt, chosen, reject, extra = self.datas[idx], self.prompts[idx], self.chosens[idx], self.rejects[idx], self.extras[idx]
    ...
    def collate_fn(self, item_list):
        ...
        rejects_masks = []
        extras = []
        for raw, chosen_id, chosen_mask, reject_id, rejects_mask, extra in item_list:
            data_raw.append(raw)
            chosen_ids.append(chosen_id)
            chosen_masks.append(chosen_mask)
            reject_ids.append(reject_id)
        ...
        reject_ids = zero_pad_sequences(reject_ids, side=padding_side, value=self.tokenizer.pad_token_id)
        rejects_masks = zero_pad_sequences(rejects_masks, side=padding_side)
        return data_raw, chosen_ids, chosen_masks, reject_ids, rejects_masks, extras
