...
def eval(args):
    ...
    all_eval_model_dir           = args.eval_model_dir
    all_eval_dataset_list        = args.eval_dataset_list
    all_eval_model_pretrain_type = args.eval_model_pretrain_type
    all_eval_model_data_type     = args.eval_model_data_type
    all_eval_model_grad_type     = args.eval_model_grad_type
    all_eval_model_cap_type      = args.eval_model_cap_type
    all_eval_model_lambda_type   = args.eval_model_lambda_type
    all_eval_model_rs_type       = args.eval_model_rs_type
    all_eval_model_tau           = args.eval_model_tau
    ...
    model_dir_list = []
    for eval_model_pretrain_type in all_eval_model_pretrain_type.split(' '):
        for eval_model_data_type in all_eval_model_data_type.split(' '):
            # for eval_model_grad_type in all_eval_model_grad_type.split(' '):
                # for eval_model_cap_type in all_eval_model_cap_type.split(' '):
                
            for eval_model_tau in all_eval_model_tau.split(' '):
                for eval_model_lambda_type in all_eval_model_lambda_type.split(' '):
                    for eval_model_rs_type in all_eval_model_rs_type.split(' '):
                        model_dir = f"{all_eval_model_dir}/{eval_model_pretrain_type}-{eval_model_data_type}-rm-nce-{eval_model_lambda_type}-tau_{eval_model_tau}-rs_{eval_model_rs_type}{exp_postfix}"
                        # print(model_dir)
                        if (os.path.exists(model_dir) and os.path.isdir(model_dir) and os.path.isfile(os.path.join(model_dir, "tokenizer_config.json"))):
                            model_dir_list.append(model_dir)
                        # else:
                        #     print(f"Can't find {model_dir} or it isn't a correct directory :(")
...
if __name__ == "__main__":
    ...
    # Eval only
    parser.add_argument("--eval_model_dir", type=str, default=None, help="Path to the evaluation model directory")
    parser.add_argument("--eval_dataset_list", type=str, default=None, help="List of evaluation datasets")
    parser.add_argument("--eval_model_pretrain_type", type=str, default=None, help="Pretrain type of the evaluation model")
    parser.add_argument("--eval_model_data_type", type=str, default=None, help="Data type of the evaluation model")
    parser.add_argument("--eval_model_grad_type", type=str, default=None, help="Gradient type of the evaluation model")
    parser.add_argument("--eval_model_cap_type", type=str, default=None, help="Cap type of the evaluation model")
    parser.add_argument("--eval_model_lambda_type", type=str, default=None, help="Lambda type of the evaluation model")
    parser.add_argument("--eval_model_rs_type", type=str, default=None, help="RS type of the evaluation model")
    parser.add_argument("--eval_model_tau", type=str, default=None, help="RS type of the evaluation model")

    parser.add_argument("--exp_postfix", type=str, default="", help="exp_postfix")
